...
grep -Fqx "go list ./... | grep -v '/node_modules/' | grep -v '/.worktrees/' | xargs go vet" <<<"$output" || fail "missing filtered lint command"
grep -Fqx "go list ./... | grep -v '/node_modules/' | grep -v '/.worktrees/' | xargs go test" <<<"$output" || fail "missing filtered test command"
...
grep -Fqx 'bash scripts/tests/codex-odin-task-test.sh' <<<"$output" || fail "missing codex task wrapper test"
