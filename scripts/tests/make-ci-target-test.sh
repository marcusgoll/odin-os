...
grep -Fqx "go test ./tests/integration -run 'TestAlphaAcceptance|TestProject(Workspace)?CLIIntegration' -count=1 -v" <<<"$output" || fail "missing alpha acceptance command"
