...
default_browser_lib="/home/orchestrator/odin-orchestrator/scripts/odin/lib/browser-access.sh"
...
resolve_browser_lib() {
    ...
    if [[ -n "${ODIN_ORCHESTRATOR_ROOT:-}" ]]; then
        printf '%s/scripts/odin/lib/browser-access.sh\n' "${ODIN_ORCHESTRATOR_ROOT%/}"
        return 0
    fi
    printf '%s\n' "${default_browser_lib}"
}
...
tool_key = str(request.get("tool_key") or "browser_pbs_session").strip() or "browser_pbs_session"
...
if [[ "${request_fields[0]:-error}" != "ok" ]]; then
    emit_json "failed" "browser_pbs_session" "${request_fields[1]:-invalid request}" '{"reason":"invalid_request"}'
    exit 0
fi
...
if ! browser_server_start --headless >/dev/null 2>&1; then
    emit_json "failed" "${tool_key}" "Unable to start trusted browser session." "$(python3 - "${bid_period}" "${workflow_key}" "${session_id}" <<'PY'
import json
import sys
...
if ! browser_load_session "${service_name}" >/dev/null 2>&1; then
    emit_json "failed" "${tool_key}" "No reusable trusted browser session is available for ${service_name}." "$(python3 - "${bid_period}" "${workflow_key}" "${session_id}" <<'PY'
import json
import sys
...
if not browser_ok or not page_ok:
    status = "failed"
    session_state = "unavailable"
    summary = f"Trusted browser session is not ready for the {bid_period} PBS workflow."
    ...
    summary = f"Trusted browser session requires login before the {bid_period} PBS workflow can run."
    evidence = ["server_healthy", "session_loaded", "login_required"]
else:
    status = "completed"
    session_state = "ready"
    summary = f"Validated trusted browser session state for the {bid_period} PBS workflow."
