...
**Supervised Agency Mode**:
The narrow, revocable **Agency Orchestrator** operating mode used for Stage 7 24/7 proof. It may create durable Odin-owned **Work Items**, **Run Attempts**, scheduler decisions, worktree leases, and **Human Review Handoffs** only for eligible low-risk work while preserving human approval, concurrency, kill-switch, recovery, and forbidden-path controls.
_Avoid_: Full autonomy, autonomous merge mode, autonomous deploy mode, unrestricted scheduler
...
- **Supervised Agency Mode** is the canonical Stage 7 24/7 operating term. It is a constrained **Agency Orchestrator** mode, not a separate product, daemon authority, top-level autonomy mode, or replacement for the **Delivery Workflow**.
- **Supervised Agency Mode** may persist durable Odin runtime state needed for restart recovery and duplicate-dispatch prevention, including **Work Items**, **Run Attempts**, scheduler decisions, active worktree leases, and **Human Review Handoffs**.
- **Supervised Agency Mode** must preserve `maxConcurrentTasks: 1`, `dryRun: false`, and `requireHumanApproval: true` until a later locked decision changes the mode.
- **Supervised Agency Mode** must not autonomously merge, deploy, mutate protected or forbidden paths, change token policy, raise concurrency, broaden eligible labels, or bypass human review.
- **Supervised Agency Mode** must stop or refuse dispatch when the kill switch is active, when restart recovery cannot prove ownership of prior work, or when an eligible issue would touch a forbidden path or non-low-risk area.
- Stage 7 eligibility requires both `odin:ready` and `safety:low-risk` labels plus an Odin-owned local path-scope preflight before worker dispatch. Labels are necessary intake hints, not sufficient dispatch authority.
- Stage 7 local path-scope preflight may pass only when the planned work is limited to `docs/`, `prompts/`, `fixtures/`, or non-sensitive tests. If the scope cannot be classified, touches a forbidden path, or may affect runner code, workspace deletion, GitHub token logic, deployment, CI secrets, or dashboard auth, Odin must refuse dispatch and leave the item for human review.
- Stage 7 non-sensitive tests are test files outside forbidden paths and outside security, runner, workspace deletion, GitHub token, deployment, CI secret, dashboard auth, or other protected-control concerns. A `*_test.go` file inside a forbidden package, or a test that exercises protected behavior, is sensitive for Stage 7 and must be refused for supervised dispatch.
- Stage 7 kill-switch state, scheduler decisions, duplicate-dispatch claims, restart-recovery observations, and active dispatch ownership must be persisted in SQLite runtime state. Process memory, logs, GitHub labels, JSON artifacts, or config files may provide evidence or defaults, but they must not outrank SQLite for mutable Stage 7 control state.
- Stage 7 static conservative defaults, including `maxConcurrentTasks: 1`, `requireHumanApproval: true`, allowed labels, forbidden paths, and the initial low-risk scope policy, belong in reviewed config rather than mutable scheduler state. Runtime state records observed decisions and operator control changes against those defaults.
- Stage 7 **Supervised Agency Mode** must expose operator controls through the canonical `odin work supervise ...` surface. Initial required subcommands are `status`, `start`, `stop`, `queue`, and `recover`; `odin serve` may host the long-running loop, but it must not become the human control surface or a parallel agency authority.
- A Stage 7 supervised **Run Attempt** may finish at **Human Review Handoff** after Odin creates a draft pull request and records CI, QA, review, and handoff evidence. The owning **Work Item** must remain non-terminal until a human explicitly records merge, rejection, requested changes, or follow-up work.
...
- Stage 6 live docs-only PR proof has a narrow GitHub mutation budget: push one approved task branch, create one pull request, and add Odin-authored Stage 6 evidence comments on that pull request. It must not change issue labels, issue state, PR review approval state, merge state, release/deploy workflow state, scheduler state, or any unrelated GitHub resource.
- Stage 6 live docs-only PR proof must create a draft pull request. The PR title, body, or evidence comments must identify it as a Stage 6 live docs-only proof and state that human merge is required.
- Stage 6 "no deployment happens" requires both zero Odin release/deploy workflow dispatches and a read-only workflow-run audit showing that no deployment-class workflow ran for the Stage 6 branch or pull request. The live command may read workflow runs for evidence, but it must not trigger, rerun, approve, cancel, or mutate workflow runs.
- Stage 6 live docs-only PR proof is command-local evidence only. It may emit JSON and local artifacts, but it must not create durable **Work Items**, **Run Attempts**, approvals, worktree leases, scheduler jobs, or PR handoff rows.
- Stage 6 live docs-only PR proof must boundedly wait for CI completion rather than only returning a run URL. Its JSON report must include the `make odin-e2e-local` workflow run URL, conclusion, and timeout status.
- Stage 6 live docs-only PR proof must be idempotent for the approved issue and branch. If the branch and matching draft pull request already exist, the live command must reuse them; if Stage 6 evidence comments with stable markers already exist, it must not duplicate them. If the docs-only diff changed since the prior proof, the command must fail unless a future explicitly approved rerun mechanism is locked.
...
- `odin workspace status` and `odin workspace list` are read-only projections of active **Worktree Lease** state through the canonical workspace operator surface. Binding, adoption, attachment, handoff, live liveness probing, and stop/termination controls for **Live Execution Sessions** are still not implemented and must not be implied by the lease projection surfaces.
