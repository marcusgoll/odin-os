...
- The browser executor validates referenced browser session metadata but refuses to pass it to the adapter. `odin browser run --session-id <id>` returns `authenticated browser session attachment is not implemented; run without browser_session_id for public read-only evidence or complete the profile attach contract first` and records no goal evidence for that attempted attach.
...
The handoff URL by itself is not proof that a browser handoff service exists. Odin exposes a read-only HTTP metadata inspection route, and the homelab PWA ingress may route that exact metadata path for operator review. When a linked NoVNC runner is `started`, the handoff/mobile surfaces may also show the private viewer URL recorded on the runner row. That viewer URL is operational metadata, not credential or profile proof. Operators must still distinguish metadata-only handoffs from `novnc-real-*` runner starts with `real_browser_evidence: true`.

When `ODIN_BROWSER_HANDOFF_BASE_URL` is configured, `odin browser session login-request --id <id> --json` uses that value as the default handoff base URL. An explicit `--handoff-base-url` flag takes precedence. The configured value must be a reachable operator URL, not a localhost URL from the server's point of view.

`handoff show` and `GET /browser/session/handoff?handoff_id=<id>` are read-only lookups for safe manual-login metadata. They require a handoff ID, reject missing IDs, unknown handoffs, non-`requested` login requests, expired requests, and revoked or missing linked sessions. They return the handoff ID, login request ID, session ID, session name, domain, optional account hint, expiration, request status, and `allowed_actions: manual_login_only` while no runner is active. If the linked login request has a `started` runner with a private viewer URL, they may also return `runner_status`, `runner_id`, `viewer_url`, and `real_browser_evidence`. They must not append runtime events, launch a browser, create NoVNC/Tailscale resources, write profile files, or store credential material.
...
The HTML shell is static and informational. It displays safe metadata, states whether a private attended browser viewer has been started, states that Odin is not collecting credentials, and states that login and 2FA stay manual. Dynamic values must be escaped. The page must not include external scripts, inline scripts, forms, credential inputs, password fields, or profile/session write affordances.
...
Policy must fail closed. In the current implementation, authenticated attach always fails closed before adapter execution. A future browser session may be attached to a goal only when all checks pass:
...
14. Authenticated read-only attachment: allow `odin browser run` and goal runner evidence collection to attach a verified `authenticated_readonly` profile for allowed domains only after the attach contract above is implemented. Until then, `odin browser run --session-id <id>` must continue to fail closed before adapter execution and must not record browser evidence for the attempted attach.
