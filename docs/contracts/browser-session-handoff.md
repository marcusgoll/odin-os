...
- This contract intentionally does not implement key storage, profile capture, browser launch changes, profile writes, credential collection, or read-only attachment. The implemented crypto helper is fixture-only and is not wired to browser session storage, CLI, HTTP, runner start, or profile capture.
