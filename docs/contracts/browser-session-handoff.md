...
The SQLite runner metadata store, metadata CLI, typed process-boundary skeleton, fixture-safe local process launch, and explicit real websockify/noVNC role launch are implemented. Real browser process implementation and full service wiring remain design only. This slice does not add HTTP handlers, Chrome/Chromium launch, real display/browser launch, Tailscale services, browser profile writes, cookie writes, or credential storage.
...
The `NoVNCRunner` launch path is selected only when `ODIN_BROWSER_HANDOFF_RUNNER=novnc`. It loads and validates `ODIN_NOVNC_*` launch config through `LoadNoVNCLaunchConfigFromEnv` and `ValidateNoVNCLaunchConfig`, then starts the configured `display`, `browser`, and `novnc/websockify` roles through the bounded supervisor. Display and browser roles remain fixture-safe only; command basenames are restricted to `true`, `false`, `sleep`, and `yes` even when another absolute path is present in the allowlist. The `novnc/websockify` role is fixture-safe by default, but may use a non-fixture executable only when `ODIN_NOVNC_REAL_WEBSOCKIFY=true` is set and the command is absolute, executable, allowlisted, and paired with validated private bind/base URL and bounded timeout settings. It records safe runner metadata, generates a private `viewer_url` from the validated private base URL, waits for child process completion, and returns `completed`, `failed`, or `expired`. It must not launch Chrome/Chromium, start Tailscale services, write profile data, or store credential material.
...
23. Real noVNC process boundary: start real display/VNC, browser, and noVNC/websockify under one supervisor, generate only private `viewer_url`, and prove cleanup with no persistent profile writes.
24. Real browser visible session: allow operator-attended manual login in the visible browser only after profile write policy, private viewer routing, cancellation, timeout, and audit behavior are proven.
25. Profile write gate integration: connect runner persistence only after encrypted profile storage and `CanWriteBrowserProfile(session)` allow writes.
26. Operator runbooks and overview visibility: surface session health, expiring profiles, active handoff runners, and waiting login goals in existing overview lanes if a clean projection lane exists.
