...
The default adapter is `internal/adapters/huginnbrowser.StubAdapter`, which returns deterministic `stub_local` read-only evidence and explicitly records `no_live_browser_launched` in its action log. This is not a live Huginn launch path. It must not submit forms, log in, click mutation controls, post, purchase, message, delete, or change external account state. Stub evidence is classified as `browser_proof_kind=stub_contract_only` with `real_browser_evidence=false`; operators must not credit it as real browser readiness. Browser goal work uses the existing `goal.evidence_recorded` audit event as its durable trail.
...
Browser run outputs, goal evidence payloads, and task work-evidence artifacts include `browser_proof_kind` and `real_browser_evidence`. Live adapter output is credited as `real_browser_evidence=true` only when `adapter_kind=huginn_live`, browser mode is proven by the worker mode or page result, the run completes without the not-implemented/no-live-browser markers, and screenshot or browser-mode action-log evidence is present. Live fetch-only evidence remains read-only evidence but is classified separately and is not a real browser proof.

Authenticated browser session attachment is not implemented. When a browser run references `browser_session_id` or `--session-id`, the executor validates the metadata boundary and then fails closed before adapter execution with an explicit unsupported attach error. The attempt must not record goal evidence or work evidence until the browser session attach contract is implemented.
