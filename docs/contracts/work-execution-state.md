...
| `odin work pr prepare --task <id\|key> --summary <text> --test <text> --risk <text> --command <text> [--changed-file <path>] [--dry-run\|--live --approval <id>] --json` | prepares durable PR handoff and review-selection evidence; dry-run/local mode does not call GitHub, and live mode requires an approved same-task Approval Request before using the pull request handoff mutation contract |
| `odin work pr review run --task <id\|key> --role <reviewer\|qa\|security> --summary <text> --json` | records selected reviewer, QA, or security handoff execution as a first-class Run Attempt, updates the selected PR review-result row, and leaves `run.started` / `run.finished` audit evidence without GitHub merge, deploy, or production mutation |
| `odin work approval request --task <id\|key> --kind <merge\|deploy> --json` | creates a task-scoped Approval Request for merge or deploy gate proof after PR handoff, completed selected review role Run Attempts, and delivery gate evidence exist; it does not merge, deploy, delete branches, or call external mutation APIs |
