...
| `internal/tracker/tracker.go` and `internal/tracker/github/client.go` | Canonical GitHub issue tracker adapter. | Agency service -> GitHub Issues. | GitHub API/token for configured live operations. | Token misuse and unintended writes remain risks for future mutation wiring, but the package root is no longer ambiguous. | Yes. Keep behind the tracker contract and approval-gated workflows. | Preserve and deepen as the only GitHub tracker seam. |
| Historical `internal/workspace/manager.go` | Removed placeholder workspace lease manager. | Agency service -> git worktree lease. | Filesystem/Git later. | Would duplicate established `internal/vcs/worktrees` and `internal/vcs/leases` if recreated. | No separate adapter. Use existing VCS packages. | Keep removed; use `internal/vcs`. |
...
| `internal/prompts/renderer.go` | Prompt renderer interface. | Worker/runner -> prompt templates. | Prompt files later. | Useful seam with repo-authored prompt assets; the former `src/prompts` TypeScript overlap is absent from the current tree. | Yes, if prompt assets remain repo-authored. | Refactor and connect to canonical prompt location. |
| Historical `src/agents/index.ts`, `src/prompts/index.ts`, `package.json`, `tsconfig.json` | Removed TypeScript agency scaffold. | Node/Vitest scaffold -> separate orchestration model. | Node 20, npm packages. | Would conflict with Go-native Odin-OS target if recreated. | No. | Keep absent; do not recreate TypeScript scaffolds. |
...
5. Keep GitHub tracker work in `internal/tracker`; do not add parallel tracker
   behavior under `internal/adapters/github`.
