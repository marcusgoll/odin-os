...
func TestCodexBridgeBuildsCanonicalInvokeRequest(t *testing.T) {
	t.Parallel()
	...
	got, err := bridge.ToInvokeRequest(ProviderCall{
		...
		ProviderPrompt:    "codex-specific prompt shaping",
	})
	if err != nil {
	...
	if got.RequestID != "req-1" {
		t.Fatalf("ToInvokeRequest().RequestID = %q, want %q", got.RequestID, "req-1")
	}
