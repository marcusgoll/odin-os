...
func TestRenderOverviewUsesCanonicalLanes(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Capability Catalog",
		"Capability Truth",
		"authored_assets=7 runtime_proven=1 partial=2 advisory=4 unknown=0",
		"truth kind=agent key=finance-advisor level=authored_asset implemented=false",
...
func sampleOverview() overview.View {
	...
	return overview.View{
		...
		CapabilityTruth: overview.CapabilityTruthLane{
			Wiring:              overview.WiringLive,
			AuthoredAssetCount:  7,
			RuntimeProvenCount:  1,
			PartialCount:        2,
			AdvisoryCount:       4,
			UnknownCount:        0,
			HighRiskFamilyCount: 1,
			Notes: []string{
				"Registry prompts are authored assets until runtime proof exists.",
			},
			Items: []overview.CapabilityTruthSummary{
				{
					Kind:                "agent",
					Key:                 "finance-advisor",
					Title:               "Finance Advisor",
					TruthLevel:          "authored_asset",
					CountsAsImplemented: false,
					Proof:               []string{"registry/agents"},
				},
				{
					Kind:                "agent",
					Key:                 "portal-delivery-agent",
					Title:               "Portal Delivery Agent",
					TruthLevel:          "runtime_proven",
					CountsAsImplemented: true,
					Proof:               []string{"odin companion delegate", "jobs", "runs", "delegations", "logs"},
				},
			},
		},
