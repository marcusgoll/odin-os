...
func TestMobileApprovalsAPIListsDetailAndRequiresExplicitHighRiskConfirmation(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("NewRequest mobile approvals list error = %v", err)
	}
	listRequest.Header.Set("X-Odin-Admin-Token", "mobile-token")
...
func fetchMobileApprovalDetail(t *testing.T, serverURL string, approvalID int64) mobileApprovalDetailTestView {
	t.Helper()
	request, err := http.NewRequest(http.MethodGet, fmt.Sprintf("%s/mobile/approvals/%d", serverURL, approvalID), nil)
	if err != nil {
		t.Fatalf("NewRequest mobile approval detail error = %v", err)
	}
	request.Header.Set("X-Odin-Admin-Token", "mobile-token")
	response, err := http.DefaultClient.Do(request)
	if err != nil {
