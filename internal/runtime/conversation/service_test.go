...
func TestServiceRespondRoutesByPromptAndUsesExecutorPromptOverride(t *testing.T) {
	t.Parallel()
	...
	result, err := service.Respond(context.Background(), Request{
		Scope:          scope.Resolution{Kind: scope.ScopeGlobal},
		Mode:           "ask",
		Prompt:         "audit the dashboard hierarchy",
		ExecutorPrompt: "Use the selected Odin skill while completing this task.\nSkill: Pixel Perfect UI/UX Designer\n\nTask Request:\naudit the dashboard hierarchy",
	})
	if err != nil {
		t.Fatalf("Respond() error = %v", err)
	}
	...
	if !strings.Contains(result.Answer, "Pixel Perfect UI/UX Designer") {
		t.Fatalf("Answer = %q, want executor prompt override to be used", result.Answer)
	}
}
