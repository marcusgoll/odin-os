...
func TestAlphaAcceptance(t *testing.T) {
	...
	t.Run("cli shell supports ask and act with explicit scope visibility", func(t *testing.T) {
		runtimeRoot := t.TempDir()
		driverPath := writeCodexDriverFixture(t, `{"status":"completed","output":"fixture driver handled task","external_id":"fixture-driver-1","metadata":{"lane":"driver"}}`)
		output, err := runOdinCommand(t, repoRoot, odinBinary, runtimeRoot, map[string]string{
			"ODIN_CODEX_DRIVER": driverPath,
	...
	t.Run("migration extraction from odin-orchestrator works", func(t *testing.T) {
		sourceRoot := createLegacyOrchestratorFixture(t)
